...
| **T-7** | **Ícones no rodapé** | ✅ Contato, Onde estamos e Redes sociais ganharam ícone na cor herdada. Os desenhos de linha provisórios saíram: entraram os 7 do acervo (`Icones-site/grupo-1/`), cheios, mais o WhatsApp do botão flutuante — 8 por página, nas 4. Como silhueta cheia pesa mais que traço no mesmo quadrado, `.footer-col--ic .ic` foi de 16px/opacidade .7 para 15px/.62, para a mancha do rodapé não mudar |
| **T-8** | **Ícones cheios nos cards de tratamento** | ✅ **18 de 18 — as três grades fechadas.** Oito vieram de `Icones-site/grupo-2/`, com uma plaqueta 64×64 em volta que saiu no import: o glifo estava vazado nela por `evenodd`, então tirar o retângulo externo inverte a paridade e devolve a forma cheia. Outros oito vieram de `Icones-site/faltantes/`. No CSS é `.card-ic__ic svg.ic-solido` (26px, `fill`). **Duas exceções:** *assimetria facial* não veio vetor — traço rasterizado que virou máscara de um `<rect>` em `currentColor` (`.ic-mascara`, 30px), PNG reduzido a 160px e embutido, 7 KB contra 115 KB do original; e *frenectomia* ficou com `cadeira-de-dentista.svg` por decisão do autor, já que não existe SVG de freio labial em banco nenhum. **Dois sintomas trocados** por não terem ícone possível: "desproporção entre os maxilares" virou "dor de cabeça e tensão" (Jonathas) e "alteração no disco" virou "dificuldade para mastigar" (Matheus) — ambos são queixa de paciente, não nome de achado clínico, o que aproxima a grade da linguagem do resto da página. `mastigacao.svg` aparece nas duas páginas: ninguém vê as três grades lado a lado. Sobrou sem uso `denture.svg` — era o que estava em pré-protéticas até o autor trocar por `protetico.svg`, que comunica melhor o preparo para a prótese |
