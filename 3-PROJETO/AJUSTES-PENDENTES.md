...
| **T-8** | **Ícones cheios nos cards de tratamento** | ⬜ **16 de 18 aplicados.** Oito vieram de `Icones-site/grupo-2/`, com uma plaqueta 64×64 em volta que saiu no import — o glifo estava vazado nela por `evenodd`, então tirar o retângulo externo inverte a paridade e devolve a forma cheia. Outros seis vieram de `Icones-site/faltantes/` e fecharam quase toda a página do Lucas. No CSS é `.card-ic__ic svg.ic-solido`. **Lucas fechou em 6 de 6** — frenectomia ficou com `cadeira-de-dentista.svg`, escolha do autor: não existe SVG de freio labial em banco nenhum, e o consultório é a referência mais próxima. **Faltam 2:** desproporção entre maxilares (Jonathas) e alteração no disco (Matheus). Assimetria facial entrou, mas é o único que **não veio vetor**: um traço rasterizado, virou máscara de um retângulo em `currentColor` (`.ic-mascara`, 30px). PNG reduzido a 160px e embutido — 7 KB, contra 115 KB do original. Sobrou sem destino `protetico.svg`; `dor-de-cabeca.svg` chegou repetido, o card já usa o do grupo 2. Até fechar, as três grades mostram os dois estilos juntos |
