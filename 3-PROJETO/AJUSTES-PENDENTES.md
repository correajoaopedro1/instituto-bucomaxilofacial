...
| **T-8** | **Ícones cheios nos cards de tratamento** | ⬜ **15 de 18 aplicados.** Oito vieram de `Icones-site/grupo-2/`, com uma plaqueta 64×64 em volta que saiu no import — o glifo estava vazado nela por `evenodd`, então tirar o retângulo externo inverte a paridade e devolve a forma cheia. Outros seis vieram de `Icones-site/faltantes/` e fecharam quase toda a página do Lucas. No CSS é `.card-ic__ic svg.ic-solido`. **Lucas fechou em 6 de 6** — frenectomia ficou com `cadeira-de-dentista.svg`, escolha do autor: não existe SVG de freio labial em banco nenhum, e o consultório é a referência mais próxima. **Faltam 3:** 2 do Jonathas (desproporção entre maxilares, assimetria facial) e 1 do Matheus (alteração no disco). Sobrou sem destino `protetico.svg`; `dor-de-cabeca.svg` chegou repetido, o card já usa o do grupo 2. Até fechar, as três grades mostram os dois estilos juntos |
